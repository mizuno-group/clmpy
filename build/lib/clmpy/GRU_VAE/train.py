...
class Trainer():
    ...
    def train(self,max_epoch:int):
        ...
        for epoch in range(self.epochs_run,max_epoch):
            l, l2 = self.train_epoch(epoch)
            l_v, l_v2, end = self.valid_epoch()
            if len(loss_v) == 0 or l_v < loss_v[-1]:
...
def main():
    ...
    loss_t, loss_t2, loss_v, loss_v2 = trainer.train(args.epochs)
    torch.save(trainer.best_model.state_dict(),os.path.join(args.experiment_dir,"best_model.pt"))
    os.remove(trainer.ckpt_path)
    if args.plot:
        plot_loss(loss_t,loss_t2,loss_v,loss_v2,dir_name=args.experiment_dir)
