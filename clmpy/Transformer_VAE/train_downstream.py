...
from .model import TransformerVAE_MLP, KLLoss
...
class Trainer():
    ...
    def _valid_batch(self,source,target,target_mlp,device):
        ...
        r_list = d_rounded.squeeze(1).tolist()  
        d_list = torch.sigmoid(out_mlp).squeeze(1).tolist()  

        for t, r, d in zip(t_list, r_list, d_list):
            row.append([d, r, t])  # ここで `[d[0], r[0], t]` ではなく `[d, r, t]`
    ...
    def train(self,args):
        ...
        while end == False:
            ...
            l_t, l_v, l_t2, l_v2, end = self._train(train_data,log=True)
            lt.extend(l_t)
            lv.extend(l_v)
            lt2.extend(l_t2)
            lv2.extend(l_v2)
