...
# --- RoPE Implementation (Brushed Up) ---
...
def apply_rotary_pos_emb(q: torch.Tensor, k: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Args:
        q (torch.Tensor): Query tensor of shape [B, Num_H, L, Hdim].
        k (torch.Tensor): Key tensor of shape [B, Num_H, L, Hdim].
        cos (torch.Tensor): Cosine cache of shape [1, 1, L, Hdim].
        sin (torch.Tensor): Sine cache of shape [1, 1, L, Hdim].

    Returns:
        A tuple containing the rotated query and key tensors.
    """
    # cos, sin are sliced to match the length of q and k in the caller
...
class RotaryEmbedding(nn.Module):
    ...
    This module pre-computes the sine and cosine frequencies and caches them.
    """
    def __init__(self, dim: int, n_positions: int = 2048, base: int = 10000, device: Optional[torch.device] = None):
        ...
        # Note: checking device inside init might require careful handling if moving models later,
        # but buffer registration handles most cases.
        inv_freq = 1.0 / (self.base ** (torch.arange(0, self.dim, 2).float().to(device) / self.dim))
        self.register_buffer("inv_freq", inv_freq)

        # Cache for sine and cosine values
    ...
    def _set_cos_sin_cache(self, seq_len: int, device: torch.device):
        ...
        # Concatenate to handle both real and imaginary parts of complex numbers
        emb = torch.cat((freqs, freqs), dim=-1)
        
        # Store as non-persistent buffers (so they are not saved in state_dict checkpoints, saving space)
        self.register_buffer("cos_cached", emb.cos()[None, None, :, :], persistent=False)
        self.register_buffer("sin_cached", emb.sin()[None, None, :, :], persistent=False)

    def forward(self, x: torch.Tensor, seq_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Retrieves the cached sine and cosine values for the given sequence length.
        ...
        Returns:
            A tuple containing the cosine and sine caches sliced to seq_len.
        """
        # If the required sequence length exceeds the cached length, recompute the cache
    ...
    def forward(self, x, attention_mask=None, layer_past=None):
        ...
        if self.use_rope:
            ...
            if layer_past is not None:
                # layer_past[0] shape: [B, H, past_L, D_head]
                past_length = layer_past[0].size(-2)
            else:
                past_length = 0
            
            query_len = query.shape[2] # Current sequence length L
            
            # Get cos, sin for the TOTAL length (past + current)
            cos, sin = self.rotary_emb(x=value, seq_len=past_length + query_len)
            
            # Slice cos/sin for the CURRENT positions (from past_length to end)
...
class TransformerBlock(nn.Module):
    def __init__(self, config, scale=False):
        # Create GPT2Config and copy attributes
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        # If using RoPE, absolute positional encoding is usually removed (Identity)
        if self.use_rope:
    ...
    def memory_pool(self, memory, pad_array):
        ...
        # Handle case where all padding to avoid NaN
        sum_mask = torch.sum(padding_mask, dim=0)
        sum_mask = torch.clamp(sum_mask, min=1e-9)
        ave = torch.sum(memory * padding_mask, dim=0) / sum_mask
...
class Decoder(nn.Module):
    ...
    def forward(self, x, latent, layer_past=None):
        ...
        # Latent injection (add latent vector to sequence)
        ...
        # Return hidden_states and presents (kv-cache) for autoregressive generation
...
class TransformerLatent(nn.Module):
    ...
    def forward(self, src, tgt, past=None):
        ...
        outputs, presents = self.decoder(tgt, latent, layer_past=past)
...
class downstream_MLP(nn.Module):
    def __init__(self, config):
        ...
        # careful: modify a copy of layer_dim to avoid side effects if config is reused
        layer_dim = list(config.layer_dim)
...
class TransformerLatent_MLP(nn.Module):
    ...
    def forward(self, src, tgt, past=None):
        ...
        # Decoder returns (logits, presents)
        out, presents = self.decoder(tgt, latent, layer_past=past)

        out_d = self.mlp(latent)

        # Return format: logits, downstream_output, latent_vector
        # (presents ignored here based on original return signature, but can be added if needed)
